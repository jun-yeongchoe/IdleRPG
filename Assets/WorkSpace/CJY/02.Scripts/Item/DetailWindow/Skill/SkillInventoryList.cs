...
public class SkillInventoryList : MonoBehaviour
{
    [Header("UI Ref")]
    ...
    public BaseSkillWindow detailWindow;
    ...
    {
        SkillMerge Merge = GetComponent<SkillMerge>();
        Merge.MergeAllSkill();
        RefreshInvenUI();
    }
    ...
    public void RefreshInvenUI()
    {
        ...
        var sortedInven = DataManager.Instance.SkillDict.OrderBy(x => x.Key);
        ...
        foreach(var item in sortedInven)
        {
            ...
            if(itemPrefab != null)
            {
                if(itemPrefab.TryGetComponent(out SkillDataSo skillData))
                {
                    ...
                    btn.onClick.AddListener(() =>
                    {
                        ...
                        detailWindow.UpdateUI(skillData, itemLevel, itemQuantity);
                    });
